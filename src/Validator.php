...
class Validator
{
	...
	public static function langDir($dir = null)
	{
		...
		return static::$_langDir ?: __DIR__ . '/validator/lang';
	}
	...
	protected function validateRequiredWith($field, $value, $params, $fields)
	{
		...
		if (isset($params[0])) {
			...
			foreach ($reqParams as $requiredField) {
				...
				if (isset($requiredFieldValue) && ( ! is_string($requiredFieldValue) || trim(
							$requiredFieldValue
						) !== '')) {
					if ( ! $allRequired) {
		...
		if (isset($this->_labels[$field])) {
			...
			if (is_array($params)) {
				...
				foreach ($params as $k => $v) {
					$tag = '{field' . $i . '}';
					$label = isset($params[$k]) && (is_numeric($params[$k]) || is_string(
							$params[$k]
						)) && isset($this->_labels[$params[$k]]) ? $this->_labels[$params[$k]] : $tag;
